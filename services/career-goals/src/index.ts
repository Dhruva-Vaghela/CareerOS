...
import { careerGoalRouter } from './routes/careerGoal.routes.js';

const logger = createLogger('career-goals-service');

async function bootstrap() {
  logger.info({ env: config.NODE_ENV }, 'Starting CareerOS Career Goals service...');
  ...
  app.use('/api/v1/career-goals', careerGoalRouter);

  app.use(globalErrorHandler);

  const server = app.listen(config.PORT, () => {
    logger.info({ port: config.PORT }, 'Career Goals Server listening');
  });
...
if (process.env.NODE_ENV !== 'test') {
  bootstrap().catch((err) => {
    logger.fatal({ err }, 'Career Goals service failed to bootstrap');
    process.exit(1);
  });
}
